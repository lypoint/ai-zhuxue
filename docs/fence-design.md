...
- `fence_bank.yaml`：三组样例（study_allow / entertainment_rewrite / sensitive_reject），当前为演示集。**扩展规范**：
  - 每库 ≥300 题（study_allow 建议扩到 ≥600：学科 50% + 方法规划 15% + 易误拦截边缘样本 20% + 口语化 15%）；
...
- 评测：`FENCE_MODE=llm GLM_API_KEY=… python -m tools.eval_fence`，输出各组通过率并对照门槛。
...
heuristic 模式的 61% 学习应答率在语义分类下全部消除（含"二战怎么爆发""唐诗宋词区别"等此前漏判样本）。**围栏验收的指标口径首次真实达标**。剩余差距仅在题库规模（当前 69+8 题，验收要求每库 ≥300 题——需教研扩展）与对抗子集的持续扩充。
