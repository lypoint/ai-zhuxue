...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (mid != null) {
        _bubbles[_bubbles.length - 1] = _Bubble(
          'assistant',
          _bubbles.last.text,
          messageId: mid,
        );
            ...
            children: [
              ...
              Expanded(
                child: _sessions == null
                    ...
                    ? Center(
                        child: Text(_search.isEmpty ? '还没有聊天记录' : '没有匹配的会话'),
                      )
                    : ListView.builder(
                        itemCount: _filteredSessions.length,
                        itemBuilder: (_, i) {
                          final c =
                              _filteredSessions[i] as Map<String, dynamic>;
                          final selected =
                              c['conversation_id'] == _conversationId;
                          return ListTile(
                            selected: selected,
                            leading: Icon(
                              selected
                            ...
                            subtitle: Text(
                              '${c['message_count']} 条 · ${(c['last_time'] ?? '').toString().replaceAll('T', ' ').sliceSafe(0, 16)}',
                              style: const TextStyle(fontSize: 11),
                            ),
                            onTap: () =>
                                _openSession(c['conversation_id'] as int),
