...
const apiBase = String.fromEnvironment('API_BASE', defaultValue: 'http://10.0.2.2:8100');
...
class Api {
 ...
 Future<void> loadToken() async {
  ...
  Future<void> _saveToken(String? t) async {
    _token = t;
    final prefs = await SharedPreferences.getInstance();
    if (t == null) { await prefs.remove('token'); } else { await prefs.setString('token', t); }
  }

  bool get hasToken => _token != null;
  void clearToken() { _token = null; }

  Future<dynamic> _sendRaw(String method, String path, {Map<String, dynamic>? body}) async {
    final uri = Uri.parse('$apiBase$path');
    final headers = {'Content-Type': 'application/json', if (_token != null) 'Authorization': 'Bearer $_token'};
    ...
    if (resp.statusCode >= 400) {
      throw ApiException(resp.statusCode, (data['detail'] ?? '请求失败').toString());
    }
    return data;
  }

  Future<Map<String, dynamic>> _send(String method, String path, {Map<String, dynamic>? body}) async => (await _sendRaw(method, path, body: body)) as Map<String, dynamic>;

  Future<String> registerGuardian(String phone, String smsCode, String nickname, String realName, String idNumber) async {
    final data = await _send('POST', '/auth/guardian/register', body: {
      'phone': phone, 'sms_code': smsCode, 'nickname': nickname,
  ...
  Future<String> studentLogin(String bindCode, String deviceId, String nickname) async {
    final data = await _send('POST', '/auth/student/login', body: {
      'bind_code': bindCode, 'device_id': deviceId, 'nickname': nickname,
    });
    await _saveToken(data['token'] as String);
    return data['role'] as String;
  }

  Future<Map<String, dynamic>> familyOverview() => _send('GET', '/parent/family');
  Future<Map<String, dynamic>> createBindCode() => _send('POST', '/bind/code');
  Future<List<dynamic>> conversations(int studentId) async =>
      (await _send('GET', '/parent/students/$studentId/conversations')) as List<dynamic>;
  Future<List<dynamic>> messages(int conversationId) async =>
      (await _send('GET', '/parent/conversations/$conversationId/messages')) as List<dynamic>;
  Future<List<dynamic>> fenceEvents(int conversationId) async =>
      (await _send('GET', '/parent/conversations/$conversationId/fence-events')) as List<dynamic>;
  Future<void> updateSettings(int dailyCap, bool reviewEnabled,
          {bool quietEnabled = true, int quietStart = 22, int quietEnd = 6,
           int dailyMinutesCap = 60, bool notifyFence = true}) =>
      _send('PUT', '/parent/settings', body: {
  ...
  Future<List<dynamic>> chatHistory(int conversationId) async =>
      (await _send('GET', '/chat/conversations?conversation_id=$conversationId')) as List<dynamic>;

  /// 学生端恢复最近对话：{conversation_id: int|null, messages: [...]}
  Future<Map<String, dynamic>> latestConversation() => _send('GET', '/chat/latest');
  ...
  Future<List<dynamic>> conversationMessages(int conversationId) async =>
      (await _sendRaw('GET', '/chat/conversations?conversation_id=$conversationId')) as List<dynamic>;
  ...
  /// 置顶/取消置顶会话
  Future<void> pinSession(int conversationId, bool pinned) =>
      _send('PUT', '/chat/sessions/$conversationId/pin', body: {'pinned': pinned});
  ...
  // ---------- 学段（P2 分龄） ----------
  Future<void> setGradeBand(int studentId, String band) =>
      _send('PUT', '/parent/students/$studentId/grade-band', body: {'grade_band': band});

  /// 修改孩子昵称（P2 遗漏补齐）
  Future<void> setStudentNickname(int studentId, String nickname) =>
      _send('PUT', '/parent/students/$studentId/nickname', body: {'nickname': nickname});

  /// 孩子收藏（学习沉淀，家长可见）
  Future<List<dynamic>> studentFavorites(int studentId) async =>
      (await _sendRaw('GET', '/parent/students/$studentId/favorites')) as List<dynamic>;

  /// 审查内容搜索（标题+消息全文）
  Future<List<dynamic>> searchConversations(int studentId, String query) async =>
      (await _sendRaw('GET', '/parent/students/$studentId/search?q=$query')) as List<dynamic>;
  ...
  Future<void> heartbeat(int seconds) => _send('POST', '/chat/heartbeat', body: {'seconds': seconds});
  ...
  // ---------- 订阅（P0） ----------
  Future<Map<String, dynamic>> subscription() => _send('GET', '/parent/subscription');
  Future<Map<String, dynamic>> paySubscription() => _send('POST', '/parent/subscription/pay');

  // ---------- 通知中心（P0） ----------
  Future<Map<String, dynamic>> notifications({bool unreadOnly = false}) =>
      _send('GET', '/parent/notifications${unreadOnly ? '?unread_only=true' : ''}');
  ...
  Future<Map<String, dynamic>> sendChat(int? conversationId, String content) =>
      _send('POST', '/chat', body: {'conversation_id': conversationId, 'content': content});
  ...
  Future<Map<String, dynamic>> sendChatStream(
      ...
      void Function(Map<String, dynamic> meta)? onMeta}) async {
    ...
    try {
      ...
      await for (final raw in resp.stream.transform(utf8.decoder).transform(const LineSplitter())) {
